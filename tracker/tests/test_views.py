...
class ViewTests(TestCase):
    ...
    def test_annotation_workflow_and_audit_endpoints_for_reviewer(self):
        ...
        audit_response = reviewer_client.get(
            reverse('tracker:session_audit_json', args=[session.pk])
        )
    ...
    def test_project_bundle_export(self):
        self.project.sessions.create(
            title='Bundle session', observer=self.user, session_kind='live'
        )
    ...
    def test_session_media_analysis_endpoint_and_extra_exports(self):
        ...
        response = self.client.get(
            reverse('tracker:session_media_analysis_json', args=[session.pk])
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertIn(
            'CREATE TABLE IF NOT EXISTS pybehaviorlog_event_export',
            sql_response.content.decode('utf-8'),
        )
    ...
    def test_project_import_boris_json_view(self):
        payload = {
            ...
            'subject_groups': [
                {'name': 'Imported group', 'description': '', 'color': '#123456', 'sort_order': 1}
            ],
            'subjects': [
                {
            ...
            'observation_templates': [
                {
                    ...
                    'behaviors': ['Imported behavior'],
                    'modifiers': [],
                    'subjects': ['Imported subject'],
                    'variable_definitions': ['Weight'],
                }
            ],
            'sessions': [
                {
        ...
        upload = SimpleUploadedFile(
            'project.json', json.dumps(payload).encode('utf-8'), content_type='application/json'
        )
        response = self.client.post(
        ...
        self.assertTrue(
            self.project.observation_templates.filter(name='Imported template').exists()
        )
    ...
    def test_workflow_fix_unpaired_states_action(self):
        ...
        self.client.post(
            reverse('tracker:event_create_api', args=[session.pk]),
            data=json.dumps(
                {'behavior_id': state_behavior.pk, 'timestamp_seconds': 2.0, 'event_kind': 'start'}
            ),
            content_type='application/json',
        )
        response = self.client.post(
    ...
    def test_project_import_boris_json_accepts_mapping_shapes(self):
        payload = {
            ...
            'groups': {
                'Adults': {'description': 'adult group', 'color': '#123456', 'sort_order': 1}
            },
            'subjects': {
                'Cow A': {'key': 'a', 'color': '#654321', 'sort_order': 1, 'groups': ['Adults']}
            },
    ...
    def test_segment_batch_assign_and_review_queue_filters_and_export(self):
        session = self.project.sessions.create(
            title='Batch session', observer=self.user, session_kind='live'
        )
    ...
    def test_session_export_json_contains_segments(self):
        session = self.project.sessions.create(
            title='Segment export', observer=self.user, session_kind='live'
        )
