...
| Native `.boris` project/session export | Compatible profile | `build_native_boris_project_payload`, `project_export_boris_native`, `session_export_boris_native` | BORIS 9.12.1 still writes `project_format_version` as `7.0`; export profiles target BORIS 7/8/9 by adjusting optional media and observation fields, including BORIS 8/9 `observation time interval`, `media_info.frames`, and BORIS 9 `media_info.display` when applicable. |
